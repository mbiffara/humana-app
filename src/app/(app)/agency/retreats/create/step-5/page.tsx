/** Agency retreat wizard step 5 — image gallery.
 *  Blob previews appear instantly, uploads run in the background,
 *  first image is the cover. Min 3, max 10 images. */
"use client";
...
export default function AgencyRetreatGalleryStep() {
  const { t } = useLocale();
  ...
  function handleFiles(files: FileList | File[]) {
    ...
    for (const file of selected) {
      try {
        ...
        // invalid type/size — skip silently
      }
    }
  ...
  return (
    <section className="rounded-xl border border-humana-line bg-white p-8 shadow-sm animate-fade-in-up">
      ...
      <label
        htmlFor="agency-retreat-gallery-input"
        onDragOver={(e) => {
          e.preventDefault();
          setDragOver(true);
        }}
      ...
      <input
        ref={inputRef}
        id="agency-retreat-gallery-input"
        type="file"
