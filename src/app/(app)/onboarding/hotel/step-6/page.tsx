/** Step 6 — Review & Status. Summary of everything saved in steps 1–5 with
...
import { spaceTypeLabel } from "@/lib/space-catalog";
import {
...
export default function HotelWizardStep6() {
  ...
  return (
    <div className="flex justify-center px-16 py-16 pb-28 animate-fade-in-up">
      ...
      <p className="mt-2 text-[15px] leading-[22px] text-humana-muted">{h.step6Sub}</p>
      ...
      <div className="mt-6 flex flex-col gap-5">
        ...
        {/* Common spaces */}
        <SectionCard title={h.reviewCommonSpaces} editLabel={h.reviewEdit} onEdit={editStep(3)}>
          {state.commonSpaces.length > 0 ? (
            <div className="flex flex-col gap-2">
              {state.commonSpaces.map((space) => (
                <div
                  key={space.localId}
                  className="flex items-center justify-between gap-4 rounded-[6px] border border-humana-line/70 px-3 py-2.5"
                >
                  <p className="truncate text-[14px] font-medium text-humana-ink">{space.name}</p>
                  <p className="shrink-0 text-[13px] text-humana-muted">
                    {spaceTypeLabel(cs, space.spaceType, space.spaceTypeOther) ?? "—"}
                  </p>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-[14px] text-humana-muted">{cs.empty}</p>
          )}
        </SectionCard>

        {/* Amenities */}
        <SectionCard title={h.reviewAmenities} editLabel={h.reviewEdit} onEdit={editStep(4)}>
        ...
        {/* Photos */}
        <SectionCard title={h.reviewPhotos} editLabel={h.reviewEdit} onEdit={editStep(5)}>
